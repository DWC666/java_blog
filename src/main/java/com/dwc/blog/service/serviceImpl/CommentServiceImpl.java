...
import com.dwc.blog.dao.CommentRepository;
...
public class CommentServiceImpl implements CommentService {
    ...
    public List<Comment> listCommentByBlogId(Long blogId) {
        Sort sort = Sort.by(Sort.Direction.DESC,"createTime"); //按评论时间倒序
    ...
    public Comment saveComment(Comment comment) {
        ...
        if(parentId != -1){
            comment.setParentComment(repository.getOne(parentId));
        }else{
    ...
    private void combineChildren(List<Comment> comments) {
        for (Comment comment : comments) {
            List<Comment> replys1 = comment.getChildComments(); //获取第一代子节点
    ...
    private void recursively(Comment comment) {
        ...
        if (comment.getChildComments().size()>0) {
            List<Comment> replys = comment.getChildComments(); //获取第一代子节点
